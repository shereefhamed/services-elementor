		...
		<script type="text/javascript">
			(function($){
				...
				$.ajax({
					...
					method: 'POST',
					data: {
						action: 'get_elementor_widget_posts',
						post_type: postType,
					},
					success: function(response) {
								...
								html += `
									<div class="post-slide">
										<div class="post-img">
											...
											<a href="${post.link}" class="over-layer"><i class="fa fa-link"></i></a>
										</div>
										<div class="post-content">
											<h3 class="post-title">
						...
						setTimeout(function() {
							if (typeof $slider.owlCarousel === 'function') {
								...
								console.warn('Owl Carousel is not loaded yet.');
							}
						}, 100);
						} else {
							$('#news-slider').html('<p>No posts found.</p>');
						}
					},
					error: function() {
						$('#news-slider').html('<p>Failed to load posts.</p>');
					}
				});
			})(jQuery);
